...
const makeSVG = answers => {
    ...
    switch (answers.shape) {
        ...
        case "triangle":
            ...
            break;
        case "polygon":
            logoShape = new shape.Polygon(answers.strokeColor, fillColor, parseInt(answers.points));
        }
    ...
    answers.textColor = checkHex(answers.textColor);
    // builds the svg markup
    let text = logoShape.svgHeader() + '\n';
    //handles the gradient markup if the color type is gradient
    if (answers.shapeColorType === "gradient") {
        // adds the pound (#) symbol if the color is a hex
        answers.shapeColor = checkHex(answers.shapeColor);
        answers.shapeColor2 = checkHex(answers.shapeColor2);
